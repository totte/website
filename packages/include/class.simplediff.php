...
// Returns coloured diff table fields from two given arrays
function html_diff($old, $new)
{
    ...
    foreach ($diff as $i=>$line)
    {
        ...
        elseif ($type == simpleDiff::CHANGED)
        {
            ...
            $old = str_replace('  ', ' ', $old);
            $old = str_replace('-] [-', ' ', $old);
            $old = preg_replace('!\[-(.*)-\]!U', '<delete>\\1</delete>', $old);

            // Don't show old things in added line
            $new = preg_replace('!\[-(?:.*)-\]!U', '', $lineDiff);
            $new = str_replace('  ', ' ', $new);
            $new = str_replace('+} {+', ' ', $new);
            $new = preg_replace('!\{\+(.*)\+\}!U', '<insert>\\1</insert>', $new);
        }
