...
require 'include/class.simplediff.php';
...
$mode = (isset($_GET['mode'])) ? $_GET['mode'] : "arch";
cpHeader();
...
switch($mode) {
    case "branch":
        if(noParent(prunedRepo($diff))=="bundles") {
            ...
            $i686_testing_files = getDatabaseFiles($diff."-testing/i686");
            $x86_64_master_files = getDatabaseFiles($diff."/x86_64");
            $x86_64_testing_files = getDatabaseFiles($diff."-testing/x86_64");
        }
...
echo "<br /><br />";
echo "<table class=cctable border=0;>";
echo "<tr><td><a href=\"index.php?subdir=".rawurlencode($diff)."\">".$messages["edt12"]."</a></td></tr>";
echo "</table>";
cpFooter();
?>
