       ...
       <div class='serendipity_entryFooter'>
		{if $entry.categories}
			...
			{else}<a href="{$entry.link}">{$entry.title|@default:$entry.body|truncate:40:" ..."}</a> {$CONST.POSTED_BY} <a href="{$entry.link_author}">{$entry.author}</a>
                ...
                {else}
                    {$CONST.AT}
                {/if} {if $dategroup.is_sticky}{$entry.timestamp|@formatTime:DATE_FORMAT_ENTRY}{else} {$entry.timestamp|@formatTime:'%H:%M'}{/if}
		{/if} - 
			{if $entry.is_entry_owner and not $is_preview}
                ...
                {if $entry.has_trackbacks}
                    {if $use_popups}
                        <a href="{$entry.link_popup_trackbacks}" onclick="window.open(this.href, 'comments', 'width=480,height=480,scrollbars=yes'); return false;">{$entry.label_trackbacks} ({$entry.trackbacks})</a><br />
                    {else}
